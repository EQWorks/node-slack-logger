...
const _putToSlack = async ({ slackHook, appName, channel, level, colors, errorOrMessageOrOptions, context }) => {
  try {
    let message
    let trail
    let stack
    ...
    const { data } = await api.post(slackHook, body)
    if (data !== 'ok') {
 ...
 * @property {string} [channel] - Slack channel.
...
class Logger {
  ...
  setOptions(options) {
    ...
    try {
      ...
      ({ slackHook, appName = 'My App', channel, minLevel = levels.WARNING, colors } = options)
      assert(
        typeof slackHook === 'string' && slackHook
        && (minLevel in levels || minLevel in levelNames)
        && typeof appName === 'string' && appName
        && ['undefined', 'string'].includes(typeof channel)
        && ['undefined', 'object'].includes(typeof colors) && colors !== null,
      )
    } catch (_) {
      // eslint-disable-next-line max-len
      throw new Error('Invalid arguments. Required: slackHook (string). Optional: level (string|number=2), appName (string=My App), channel (string), colors (object)')
    }
